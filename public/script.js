...
async function loadMyCats() {
    ...
    try {
        ...
        // Update count
        document.getElementById('my-cat-count').textContent = myCats.length;
        ...
        document.getElementById('myCatsContainer').innerHTML =
            '<div class="error" style="grid-column:1/-1;">Error loading your cats</div>';
    }
}
function checkAuthState() {
    ...
    if (savedUser && savedToken) {
        try {
            ...
            // ============ NEW: Show Dashboard on page load ============
            // Hide all sections
            document.querySelectorAll('.cyber-section').forEach(section => {
                section.style.display = 'none';
                section.classList.remove('active');
