...
function updateNavigation() {
    ...
    if (currentUser) {
        ...
        // Show dashboard section if we're on dashboard
        ...
        // Hide dashboard if user logs out
        if (window.location.hash === '#dashboard') {
...
document.getElementById('registerForm')?.addEventListener('submit', function(e) {
    ...
    addUserActivity('Welcome to Cat Gallery!', 'fas fa-user-plus');
    ...
    alert('Registration successful! Welcome to Cat Gallery.');
});
...
document.getElementById('loginForm')?.addEventListener('submit', function(e) {
    ...
    if (user) {
        ...
        // Add login activity
        addUserActivity('Logged in to dashboard', 'fas fa-sign-in-alt');
...
function showDashboard() {
    ...
    document.getElementById('userCatCount').textContent = (userCats[currentUser.id] || []).length;
...
function loadUserCats() {
    const container = document.getElementById('userCatsContainer');
    ...
    userCatsList.forEach(cat => {
        const catCard = document.createElement('div');
        catCard.className = 'user-cat-card';
        catCard.innerHTML = `
            <img src="${cat.img}" alt="${cat.name}" class="user-cat-img">
            <div class="user-cat-info">
                <h4>${cat.name}</h4>
                <p>${cat.description || 'No description provided.'}</p>
                <div class="user-cat-actions">
                    <button class="cyber-btn small" onclick="editUserCat('${cat.id}')">
                        <i class="fas fa-edit"></i> Edit
                    </button>
                    <button class="cyber-btn small danger" onclick="deleteUserCat('${cat.id}')">
...
// Add User Activity
function addUserActivity(action, icon) {
    ...
    userActivities[currentUser.id].unshift(activity);
    
    // Keep only last 5 activities
    if (userActivities[currentUser.id].length > 5) {
        userActivities[currentUser.id].pop();
    }
...
function loadRecentActivity() {
    ...
    if (activities.length === 0) {
        feed.innerHTML = '<div class="activity-item"><p>No recent activity</p></div>';
        return;
    }
...
// Modify addCat function to track user's cats
function addCat() {
    ...
    // If user is logged in, add to their collection
    if (currentUser) {
        if (!userCats[currentUser.id]) {
            userCats[currentUser.id] = [];
        }
        userCats[currentUser.id].push(cat);
        localStorage.setItem('userCats', JSON.stringify(userCats));
        
        // Add activity
        addUserActivity(`Added cat: ${name}`, 'fas fa-cat');
...
function editUserCat(catId) {
    ...
    if (cat) {
        ...
        // Store cat ID for update
        document.getElementById('addBtn').style.display = 'none';
        document.getElementById('editBtn').style.display = 'inline-block';
        document.getElementById('editBtn').dataset.catId = catId;
    }
}
...
function deleteUserCat(catId) {
    ...
    // Remove from global cats
    const catIndex = cats.findIndex(c => c.id === catId);
    if (catIndex > -1) {
        cats.splice(catIndex, 1);
    }
    
    // Remove from user's cats
    if (currentUser && userCats[currentUser.id]) {
        const userCatIndex = userCats[currentUser.id].findIndex(c => c.id === catId);
        if (userCatIndex > -1) {
            userCats[currentUser.id].splice(userCatIndex, 1);
        }
    }
    ...
    // Add activity
    addUserActivity('Deleted a cat from collection', 'fas fa-trash');
...
// Update Cat function
function updateCat() {
    const catId = document.getElementById('editBtn').dataset.catId;
    const catIndex = cats.findIndex(c => c.id === catId);
    ...
    // Update in user's cats if applicable
    if (currentUser && userCats[currentUser.id]) {
        const userCatIndex = userCats[currentUser.id].findIndex(c => c.id === catId);
        if (userCatIndex > -1) {
            userCats[currentUser.id][userCatIndex] = updatedCat;
            localStorage.setItem('userCats', JSON.stringify(userCats));
        }
    }
    ...
    // Add activity
    addUserActivity(`Updated cat: ${updatedCat.name}`, 'fas fa-edit');
...
function editProfile() {
    ...
    if (newUsername && newUsername.trim()) {
        ...
        updateNavigation();
        showDashboard();
        addUserActivity('Updated profile username', 'fas fa-user-edit');
...
function shareProfile() {
    ...
    navigator.clipboard.writeText(profileUrl).then(() => {
        ...
        addUserActivity('Shared profile link', 'fas fa-share-alt');
    });
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add dashboard link click handler
    ...
    // Update add cat button in dashboard
    document.getElementById('addUserCatBtn')?.addEventListener('click', function() {
        if (currentUser) {
            openModal();
        } else {
